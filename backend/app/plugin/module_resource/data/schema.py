...
class ResourceDataCreateSchema(BaseModel):
    """
    数据新增模型
    """
    status: str = Field(default="0", description='是否启用')
    name: str = Field(default=..., description='数据名称')
    type: str = Field(default=..., description='数据类型')
    format: str = Field(default=..., description='数据格式')
...
class ResourceDataUpdateSchema(ResourceDataCreateSchema):
    """
    数据更新模型
    """
    ...
...
class ResourceDataOutSchema(ResourceDataCreateSchema, BaseSchema, UserBySchema):
    """
    数据响应模型
    """
...
class ResourceDataQueryParam:
    """数据查询参数"""

    def __init__(
        self,
        name: str | None = Query(None, description="数据名称"),
