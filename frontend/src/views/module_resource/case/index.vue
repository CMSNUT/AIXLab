    ...
    <div v-show="visible" class="search-container">
      ...
      >
        <el-form-item label="案例名称" prop="name">
          <el-input v-model="queryFormData.name" placeholder="请输入案例名称" clearable />
        </el-form-item>
        ...
        <el-form-item>
          <el-button
            v-hasPerm="['module_resource:case:query']"
            type="primary"
            icon="search"
            @click="handleQuery"
          >
    ...
    <el-card class="data-table">
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_resource:case:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_resource:case:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_resource:case:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_resource:case:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_resource:case:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_resource:case:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_resource:case:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-else>
        ...
        >
          ...
          <el-form-item label="案例名称" prop="name" :required="false">
          ...
          <el-form-item label="内容" prop="content" :required="false">
...
import ResourceCaseAPI, {
  ResourceCasePageQuery,
  ResourceCaseTable,
  ResourceCaseForm,
} from "@/api/module_resource/case";
...
const selectionRows = ref<ResourceCaseTable[]>([]);
...
// 分页表单
const pageTableData = ref<ResourceCaseTable[]>([]);

// 表格列配置
const tableColumns = ref([
  { prop: "selection", label: "选择框", show: true },
  { prop: "index", label: "序号", show: true },
  { prop: "name", label: "案例名称", show: true },
...
const curdContentConfig = {
  permPrefix: "module_resource:case",
  cols: exportColumns as any,
  importTemplate: () => ResourceCaseAPI.downloadTemplateResourceCase(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await ResourceCaseAPI.listResourceCase(query);
...
// 详情表单
const detailFormData = ref<ResourceCaseTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<ResourceCasePageQuery>({
  page_no: 1,
  page_size: 10,
  name: undefined,
...
// 编辑表单
const formData = reactive<ResourceCaseForm>({
  id: undefined,
  status: undefined,
  name: undefined,
  description: undefined,
...
const rules = reactive({
  ...
  name: [{ required: false, message: "请输入案例名称", trigger: "blur" }],
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await ResourceCaseAPI.listResourceCase(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: ResourceCaseForm = {
  id: undefined,
  status: undefined,
  name: undefined,
  description: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await ResourceCaseAPI.detailResourceCase(id);
    if (type === "detail") {
    ...
    dialogVisible.title = "新增ResourceCase";
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await ResourceCaseAPI.updateResourceCase(id, { id, ...formData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await ResourceCaseAPI.createResourceCase(formData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await ResourceCaseAPI.deleteResourceCase(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
  ...
  if (selectIds.value.length) {
    ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
      ...
      .then(async () => {
        try {
          loading.value = true;
          await ResourceCaseAPI.batchResourceCase({ ids: selectIds.value, status });
          handleResetQuery();
        } catch (error: any) {
          console.error(error);
        } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    const response = await ResourceCaseAPI.importResourceCase(formData);
