...
<template>
  <div class="app-container">
    ...
    <div v-show="visible && !isSearchGlobalCollapsed" class="search-container">
      ...
      >
        <el-form-item label="绘图模块名称" prop="name">
          <el-input v-model="queryFormData.name" placeholder="请输入绘图模块名称" clearable />
        </el-form-item>
        <el-form-item label="模块英文名称" prop="code">
          <el-input v-model="queryFormData.code" placeholder="请输入模块英文名称" clearable />
        </el-form-item>
        <el-form-item label="领域" prop="field">
          <el-select v-model="queryFormData.field" placeholder="请选择领域" style="width: 180px" clearable>
        ...
        <el-form-item label="类别" prop="category">
          <el-input v-model="queryFormData.category" placeholder="请输入类别" clearable />
        </el-form-item>
        ...
        <el-form-item>
          <el-button
            v-hasPerm="['module_lab:plot:query']"
            type="primary"
            icon="search"
            @click="handleQuery"
          >
    ...
    <el-card class="data-table">
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_lab:plot:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_lab:plot:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_lab:plot:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_lab:plot:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_lab:plot:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_lab:plot:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_lab:plot:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="绘图模块名称" :span="2">
              {{ detailFormData.name }}
          </el-descriptions-item>
          <el-descriptions-item label="模块英文名称" :span="2">
              {{ detailFormData.code }}
          </el-descriptions-item>
          <el-descriptions-item label="领域" :span="2">
      ...
      <template v-else>
        ...
        >
          <el-form-item label="绘图模块名称" prop="name" :required="false">
            <el-input v-model="formData.name" placeholder="请输入绘图模块名称" />
          </el-form-item>
          <el-form-item label="模块英文名称" prop="code" :required="false">
            <el-input v-model="formData.code" placeholder="请输入模块英文名称" />
          </el-form-item>
          <el-form-item label="领域" prop="field" :required="false">
            <el-select v-model="formData.field" placeholder="请选择领域">
              <el-option v-for="dict in dictStore.getDictArray('sys_study_field')" :key="dict.dict_value" :label="dict.dict_label" :value="dict.dict_value" />
            </el-select>
          </el-form-item>
          <el-form-item label="类别" prop="category" :required="false">
...
const selectionRows = ref<LabPlotTable[]>([]);
...
// 分页表单
const pageTableData = ref<LabPlotTable[]>([]);

// 表格列配置
const tableColumns = ref([
  { prop: "selection", label: "选择框", show: true },
  { prop: "index", label: "序号", show: true },
  { prop: "name", label: "绘图模块名称", show: true },
  { prop: "code", label: "模块英文名称", show: true },
  { prop: "field", label: "领域", show: true },
  { prop: "category", label: "类别", show: true },
...
// 导出列（不含选择/序号/操作）
const exportColumns = [
  { prop: "name", label: "绘图模块名称" },
  { prop: "code", label: "模块英文名称" },
  { prop: "field", label: "领域" },
...
const curdContentConfig = {
  permPrefix: "module_lab:plot",
  cols: exportColumns as any,
  importTemplate: () => LabPlotAPI.downloadTemplateLabPlot(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await LabPlotAPI.listLabPlot(query);
...
// 详情表单
const detailFormData = ref<LabPlotTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<LabPlotPageQuery>({
  page_no: 1,
  page_size: 10,
  name: undefined,
  code: undefined,
...
// 编辑表单
const formData = reactive<LabPlotForm>({
  id: undefined,
  name: undefined,
  code: undefined,
  field: undefined,
...
const rules = reactive({
  ...
  name: [{ required: false, message: "请输入绘图模块名称", trigger: "blur" }],
  code: [{ required: false, message: "请输入模块英文名称", trigger: "blur" }],
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await LabPlotAPI.listLabPlot(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: LabPlotForm = {
  id: undefined,
  name: undefined,
  code: undefined,
  field: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await LabPlotAPI.detailLabPlot(id);
    if (type === "detail") {
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await LabPlotAPI.updateLabPlot(id, { id, ...formData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await LabPlotAPI.createLabPlot(formData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await LabPlotAPI.deleteLabPlot(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    const response = await LabPlotAPI.importLabPlot(formData);
