...
CREATE TABLE `project_subject` (
  ...
  `code` varchar(10) DEFAULT NULL COMMENT '课题编号',
  `field` varchar(10) DEFAULT '多元谱学' COMMENT '课题领域（可选：多元谱学/其他）',
  `content` longtext DEFAULT NULL COMMENT '课题简介',
  `start_date` datetime DEFAULT NULL COMMENT '开始日期',
  `end_date` datetime DEFAULT NULL COMMENT '结束日期',
  `status` varchar(10) DEFAULT '0' COMMENT '是否启用',
  `progress` varchar(6) DEFAULT '未开始' COMMENT '课题进度（可选：未开始/进行中/已完成）',
  `description` text DEFAULT NULL COMMENT '备注/描述',
  `created_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间（自动填充）',
  `updated_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间（自动刷新）',
  `created_id` int DEFAULT NULL COMMENT '创建人ID',
  `updated_id` int DEFAULT NULL COMMENT '更新人ID',
  PRIMARY KEY (`id`),
  UNIQUE KEY `ix_project_subject_uuid` (`uuid`),
  KEY `ix_project_subject_name` (`name`),
  KEY `ix_project_subject_field` (`field`),
  KEY `ix_project_subject_code` (`code`),
  KEY `ix_project_subject_status` (`status`),
...
CREATE TABLE `project_method` (
  ...
  `updated_id` int DEFAULT NULL COMMENT '更新人ID',
  PRIMARY KEY (`id`),
  UNIQUE KEY `ix_project_method_uuid` (`uuid`),
  KEY `ix_project_method_name` (`name`),
  KEY `ix_project_method_topic_id` (`topic_id`),
  KEY `ix_project_method_status` (`status`),
...
CREATE TABLE `project_result` (
  ...
  `type` varchar(50) DEFAULT NULL COMMENT '结果类型',
  ...
  `updated_id` int DEFAULT NULL COMMENT '更新人ID',
  PRIMARY KEY (`id`),
  UNIQUE KEY `ix_project_result_uuid` (`uuid`),
  KEY `ix_project_result_name` (`name`),
  KEY `ix_project_result_method_id` (`method_id`),
  KEY `ix_project_result_status` (`status`),
