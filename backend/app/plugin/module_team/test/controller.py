...
TeamTestRouter = APIRouter(route_class=OperationLogRoute, prefix='/test', tags=["测试模块"]) 

@TeamTestRouter.get("/detail/{id}", summary="获取测试详情", description="获取测试详情")
async def get_test_detail_controller(
    ...
    log.info(f"获取测试详情成功 {id}")
    return SuccessResponse(data=result_dict, msg="获取测试详情成功")

@TeamTestRouter.get("/list", summary="查询测试列表", description="查询测试列表")
...
@TeamTestRouter.post("/create", summary="创建测试", description="创建测试")
async def create_test_controller(
    ...
    log.info("创建测试成功")
    return SuccessResponse(data=result_dict, msg="创建测试成功")

@TeamTestRouter.put("/update/{id}", summary="修改测试", description="修改测试")
async def update_test_controller(
    ...
    log.info("修改测试成功")
    return SuccessResponse(data=result_dict, msg="修改测试成功")

@TeamTestRouter.delete("/delete", summary="删除测试", description="删除测试")
async def delete_test_controller(
    ...
    log.info(f"删除测试成功: {ids}")
    return SuccessResponse(msg="删除测试成功")

@TeamTestRouter.patch("/available/setting", summary="批量修改测试状态", description="批量修改测试状态")
async def batch_set_available_test_controller(
    ...
    log.info(f"批量修改测试状态成功: {data.ids}")
    return SuccessResponse(msg="批量修改测试状态成功")

@TeamTestRouter.post('/export', summary="导出测试", description="导出测试")
...
@TeamTestRouter.post('/import', summary="导入测试", description="导入测试")
async def import_test_list_controller(
    ...
    log.info("导入测试成功")
    return SuccessResponse(data=batch_import_result, msg="导入测试成功")

@TeamTestRouter.post('/download/template', summary="获取测试导入模板", description="获取测试导入模板", dependencies=[Depends(AuthPermission(["module_team:test:download"]))])
