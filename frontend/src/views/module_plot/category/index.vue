  ...
  <div class="app-container">
    ...
    <div v-show="visible && !isSearchGlobalCollapsed" class="search-container">
      ...
      >
        ...
        <el-form-item label="模块编码" prop="code">
          <el-input v-model="queryFormData.code" placeholder="请输入模块编码" clearable />
        </el-form-item>
        <el-form-item label="模块大类" prop="category">
          <el-input v-model="queryFormData.category" placeholder="请输入模块大类" clearable />
        </el-form-item>
        <el-form-item label="模块子类" prop="subcategory">
          <el-input v-model="queryFormData.subcategory" placeholder="请输入模块子类" clearable />
        </el-form-item>
        ...
        <el-form-item>
          <el-button
            v-hasPerm="['module_plot:category:query']"
            type="primary"
            icon="search"
            @click="handleQuery"
          >
    ...
    <el-card class="data-table">
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_plot:category:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_plot:category:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_plot:category:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_plot:category:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_plot:category:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        />
        <el-table-column
          v-if="tableColumns.find((col) => col.prop === 'subcategory')?.show"
          label="模块子类"
          prop="subcategory"
          min-width="140"
        />
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_plot:category:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_plot:category:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="模块编码" :span="2">
              {{ detailFormData.code }}
            </el-descriptions-item>
          <el-descriptions-item label="模块大类" :span="2">
              {{ detailFormData.category }}
            </el-descriptions-item>
          <el-descriptions-item label="模块子类" :span="2">
              {{ detailFormData.subcategory }}
            </el-descriptions-item>
          <el-descriptions-item label="模块图片" :span="2">                            
            <el-image
      ...
      <template v-else>
        ...
        >
          ...
          <el-form-item label="模块编码" prop="code" :required="false">
            <el-input v-model="formData.code" placeholder="请输入模块编码" />
          </el-form-item>
          <el-form-item label="模块大类" prop="category" :required="false">
            <el-input v-model="formData.category" placeholder="请输入模块大类" />
          </el-form-item>
          <el-form-item label="模块子类" prop="subcategory" :required="false">
            <el-input v-model="formData.subcategory" placeholder="请输入模块子类" />
          </el-form-item>
          <el-form-item label="模块图片" prop="image">
...
const selectionRows = ref<PlotCategoryTable[]>([]);
...
// 分页表单
const pageTableData = ref<PlotCategoryTable[]>([]);
...
const tableColumns = ref([
  ...
  { prop: "category", label: "模块大类", show: true },
  { prop: "subcategory", label: "模块子类", show: true },
  { prop: "image", label: "模块图片", show: true },
...
const exportColumns = [
  ...
  { prop: "category", label: "模块大类" },
  { prop: "subcategory", label: "模块子类" },
  { prop: "image", label: "模块图片" },
...
const curdContentConfig = {
  permPrefix: "module_plot:category",
  cols: exportColumns as any,
  importTemplate: () => PlotCategoryAPI.downloadTemplatePlotCategory(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await PlotCategoryAPI.listPlotCategory(query);
...
// 详情表单
const detailFormData = ref<PlotCategoryTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<PlotCategoryPageQuery>({
  page_no: 1,
  page_size: 10,
  name: undefined,
  code: undefined,
...
// 编辑表单
const formData = reactive<PlotCategoryForm>({
  id: undefined,
  name: undefined,
  code: undefined,
  category: undefined,
...
const rules = reactive({
  ...
  code: [{ required: false, message: "请输入模块编码", trigger: "blur" }],
  category: [{ required: false, message: "请输入模块大类", trigger: "blur" }],
  subcategory: [{ required: false, message: "请输入模块子类", trigger: "blur" }],
  image: [{ required: true, message: "请输入模块图片", trigger: "blur" }],
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await PlotCategoryAPI.listPlotCategory(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: PlotCategoryForm = {
  id: undefined,
  name: undefined,
  code: undefined,
  category: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await PlotCategoryAPI.detailPlotCategory(id);
    if (type === "detail") {
    ...
    dialogVisible.title = "新增PlotCategory";
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await PlotCategoryAPI.updatePlotCategory(id, { id, ...formData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await PlotCategoryAPI.createPlotCategory(formData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await PlotCategoryAPI.deletePlotCategory(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    const response = await PlotCategoryAPI.importPlotCategory(formData);
