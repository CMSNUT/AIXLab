    ...
    <div v-show="visible" class="search-container">
      ...
      >
        <el-form-item label="脚本名称" prop="name">
          <el-input v-model="queryFormData.name" placeholder="请输入脚本名称" clearable />
        </el-form-item>
        <el-form-item label="脚本类型" prop="type">
          <el-select v-model="queryFormData.type" placeholder="请选择脚本类型" style="width: 180px" clearable>
            <el-option v-for="dict in dictStore.getDictArray('sys_code_type')" :key="dict.dict_value" :label="dict.dict_label" :value="dict.dict_value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item>
          <el-button
            v-hasPerm="['module_resource:script:query']"
            type="primary"
            icon="search"
            @click="handleQuery"
          >
    ...
    <el-card class="data-table">
      ...
      <div class="data-table__toolbar">
        <div class="data-table__toolbar--left">
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_resource:script:create']"
                type="success"
                icon="plus"
            ...
            <el-col :span="1.5">
              <el-button
                v-hasPerm="['module_resource:script:delete']"
                type="danger"
                icon="delete"
            ...
            <el-col :span="1.5">
              <el-dropdown v-hasPerm="['module_resource:script:batch']" trigger="click">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-tooltip content="导入">
                <el-button
                  v-hasPerm="['module_resource:script:import']"
                  type="success"
                  icon="upload"
                  circle
            ...
            <el-col :span="1.5">
              <el-tooltip content="导出">
                <el-button
                  v-hasPerm="['module_resource:script:export']"
                  type="warning"
                  icon="download"
                  circle
      ...
      >
        ...
        >
          <template #default="scope">
            <el-button
              v-hasPerm="['module_resource:script:detail']"
              type="info"
              size="small"
              link
              icon="document"
            ...
            <el-button
              v-hasPerm="['module_resource:script:update']"
              type="primary"
              size="small"
              link
              icon="edit"
    ...
    >
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="脚本类型" :span="2">
            {{ detailFormData.type }}
          </el-descriptions-item>
      ...
      <template v-else>
        ...
        >
          ...
          <el-form-item label="脚本名称" prop="name" :required="false">
            <el-input v-model="formData.name" placeholder="请输入脚本名称" />
          </el-form-item>
          <el-form-item label="脚本类型" prop="type" :required="false">
            <el-select v-model="formData.type" placeholder="请选择脚本类型">
              <el-option v-for="dict in dictStore.getDictArray('sys_code_type')" :key="dict.dict_value" :label="dict.dict_label" :value="dict.dict_value" />
            </el-select>
          </el-form-item>
...
import ResourceScriptAPI, {
  ResourceScriptPageQuery,
  ResourceScriptTable,
  ResourceScriptForm,
} from "@/api/module_resource/script";
...
const selectionRows = ref<ResourceScriptTable[]>([]);
...
// 分页表单
const pageTableData = ref<ResourceScriptTable[]>([]);

// 表格列配置
const tableColumns = ref([
  { prop: "selection", label: "选择框", show: true },
  { prop: "index", label: "序号", show: true },
  { prop: "name", label: "脚本名称", show: true },
  { prop: "type", label: "脚本类型", show: true },
...
const curdContentConfig = {
  permPrefix: "module_resource:script",
  cols: exportColumns as any,
  importTemplate: () => ResourceScriptAPI.downloadTemplateResourceScript(),
  exportsAction: async (params: any) => {
    ...
    const all: any[] = [];
    while (true) {
      const res = await ResourceScriptAPI.listResourceScript(query);
...
// 详情表单
const detailFormData = ref<ResourceScriptTable>({});
// 日期范围临时变量
...
// 分页查询参数
const queryFormData = reactive<ResourceScriptPageQuery>({
  page_no: 1,
  page_size: 10,
  name: undefined,
  type: undefined,
...
// 编辑表单
const formData = reactive<ResourceScriptForm>({
  id: undefined,
  status: undefined,
  name: undefined,
  type: undefined,
...
const rules = reactive({
  ...
  name: [{ required: false, message: "请输入脚本名称", trigger: "blur" }],
  type: [{ required: true, message: "请输入脚本类型", trigger: "blur" }],
...
async function loadingData() {
  loading.value = true;
  try {
    const response = await ResourceScriptAPI.listResourceScript(queryFormData);
...
// 定义初始表单数据常量
const initialFormData: ResourceScriptForm = {
  id: undefined,
  status: undefined,
  name: undefined,
  type: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await ResourceScriptAPI.detailResourceScript(id);
    if (type === "detail") {
    ...
    dialogVisible.title = "新增ResourceScript";
...
async function handleSubmit() {
  ...
  dataFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (id) {
        try {
          await ResourceScriptAPI.updateResourceScript(id, { id, ...formData });
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
        ...
        try {
          await ResourceScriptAPI.createResourceScript(formData);
          dialogVisible.visible = false;
          resetForm();
          handleCloseDialog();
          handleResetQuery();
...
async function handleDelete(ids: number[]) {
  ElMessageBox.confirm("确认删除该项数据?", "警告", {
    ...
    .then(async () => {
      try {
        loading.value = true;
        await ResourceScriptAPI.deleteResourceScript(ids);
        handleResetQuery();
      } catch (error: any) {
        console.error(error);
      } finally {
  ...
  if (selectIds.value.length) {
    ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
      ...
      .then(async () => {
        try {
          loading.value = true;
          await ResourceScriptAPI.batchResourceScript({ ids: selectIds.value, status });
          handleResetQuery();
        } catch (error: any) {
          console.error(error);
        } finally {
...
const handleUpload = async (formData: FormData) => {
  try {
    const response = await ResourceScriptAPI.importResourceScript(formData);
