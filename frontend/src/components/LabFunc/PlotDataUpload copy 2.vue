      ...
      <div class="plot-data-params-box">
        <div class="plot-data-params-box-title">
          ...
          <div class="upload-actions">
            ...
            <el-button type="warning" @click="downloadSampleData">
              <el-icon>
                <Download />
              </el-icon>
              下载示例数据
...
//     // 创建示例数据
//     const sampleData = `样本ID,组别,处理前,处理后
// S001,A组,150.2,210.5
// S002,A组,168.7,225.1
// S003,B组,142.3,198.7
// S004,B组,155.6,215.3
...
const downloadSampleData = async () => {
  try {
    // 获取当前图表 code
    const code = plotStore.currentPlot?.code
    ...
    ElMessage.info('正在下载示例数据...')

    // 调用 API 下载文件
    const response = await LabPlotAPI.downloadPlotSampleData({ code })

    // 方法1：使用 Blob 和 URL.createObjectURL 下载
    // 从响应头中获取文件名
    let fileName = `${code}_示例数据.xlsx`
    ...
    // 方法2：使用 FileSaver.js 库（如果需要）
    // import { saveAs } from 'file-saver'
    // saveAs(blob, fileName)

  } catch (error: any) {
    console.error('下载示例数据失败:', error)
    ...
    if (error.response) {
      ...
      if (status === 400) {
        ElMessage.error('参数错误，请检查图表代码')
      } else if (status === 404) {
        ElMessage.error('示例数据文件不存在')
      } else if (status === 500) {
        ...
        try {
          ...
          ElMessage.error(`下载失败: ${errorData.detail || errorData.message || '未知错误'}`)
        } catch (parseError) {
          ElMessage.error(`下载失败: ${error.response.data || '未知错误'}`)
        }
      }
